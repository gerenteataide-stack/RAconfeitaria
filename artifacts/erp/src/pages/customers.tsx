  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">CRM com cadastro, historico financeiro e pontos.</p>
      </div>
      ...
      <section className="rounded-lg border bg-white p-4 shadow-sm">
        ...
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <div className="md:col-span-2"><Label>Endereco</Label><Input value={form.address} onChange={(event) => setForm({ ...form, address: event.target.value })} /></div>
          <div><Label>Bairro</Label><Input value={form.neighborhood} onChange={(event) => setForm({ ...form, neighborhood: event.target.value })} /></div>
          <div className="md:col-span-3"><Label>Observacoes</Label><Textarea rows={2} value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} /></div>
        ...
        {customers.length === 0 ? (
          ...
          <div key={customer.id} className="flex flex-col justify-between gap-3 rounded-lg border bg-white p-4 shadow-sm md:flex-row md:items-center">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{customer.whatsapp || customer.phone} - {customer.email || "sem email"} - {customer.address || "sem endereco"} - {customer.neighborhood || "sem bairro"}</p>
